...
# Initialize Cohere client using API key from config
co = cohere.Client(config['cohere']['api_key'])

# Function to initialize the conversation with cohere model by setting the system's initial prompt
def initialize_conversation():
    '''
    Initializes the conversation with cohere by setting a system prompt.
    Returns:
        list: A list containing a single dictionary with system role and message content.
    '''
...
# Function to send a chat completion request to cohere  with the given messages
def get_chat_model_completions(messages):
    '''
    Calls cohere 's chat completion API.
    Args:
    ...
    '''
     ...
     # Convert conversation into Cohere-compatible chat_history
    chat_history = []
    for msg in messages[:-1]:  # Fix here
        if msg["role"] == "user":
            chat_history.append({"user_name": "user", "text": msg["content"]})
        elif msg["role"] == "assistant":
            chat_history.append({"user_name": "assistant", "text": msg["content"]})

    # Get the most recent user message
    latest_msg = messages[-1]["content"]  # Fix here

    response = co.chat(
        model="command-r-plus",  # assuming you're using this model
        message=latest_msg,
...
#     '''
#     Calls OpenAI's moderation API to check if the input contains inappropriate content.
#     Args:
#         user_input (str): User's message.
#     Returns:
#         str: "Flagged" if the content is inappropriate, otherwise "Not Flagged".
#     '''
...
def moderation_check(user_input):
    '''
    Calls cohere's moderation API to check if the input contains inappropriate content.
    Args:
        user_input (str): User's message.
    Returns:
        str: "Flagged" if the content is inappropriate, otherwise "Not Flagged".
    ...
    #         name="Moderation Training Data",
    #         file="training_data.jsonl",
    #         description="Dataset to fine-tune moderation model"
    #     )
    # print("Dataset ID:", dataset.id)

    # # Create the classify model based on dataset
    # response = co.classify_models.create(
    #     dataset_id=dataset.id,
    #     name="moderation-model",
    #     task_description="Classify whether text is appropriate or not",
    #     output_classes=["Flagged", "Not Flagged"]
    # )
    # print("Model ID:", response.model_id)
    # return response.model_id

# use created model id
    # response = co.classify(
    #     #model="ft:abc123def456ghi789jkl",
    #     inputs=[user_input]
    # )
    # prediction = response.classifications[0].prediction
    # return "Flagged" if prediction == "Flagged" else "Not Flagged"

# The intent confirmation layer evaluates the output of the chat completion from Open AI API
def intent_confirmation_layer(response_assistant):
    prompt = f"""
You are a senior evaluator who has an eye for detail.
You are provided a string input. You need to check that the values for the following 
keys have been captured successfully: 
1. GPU intensity
2. Display Quality
3. Portability
4. Multi tasking
...
The values for all keys, except 'budget', must be 'low', 'medium', or 'high'.
The value of 'budget' must be a number.

Please note that every key should have a value and budget should be a valid number.
Return "No" if any one of the values is missing or invalid.

Here is the input string to check:
\"\"\"
{response_assistant}
\"\"\"
"""
    ...
    # The assistant's response text should be either "Yes" or "No"
...
#     '''
#     Extracts the user's laptop preferences from the assistant's response and formats it correctly.
#     Args:
#         response_assistant (str): Assistant's response.
#     Returns:
...
def get_user_requirement_string(response_assistant):
    prompt = f"""
You are given a string where the user requirements for the keys 
('GPU intensity','Display quality','Portability','Multitasking','Processing speed','Budget') 
have been captured inside. The values for all keys, except 'budget', will be 'low', 'medium', or 'high' 
and the value of 'budget' will be a number.

You have to output a string that captures only the user intent in this exact format:

I need a laptop with high GPU intensity, medium display quality, high portablity, high multi tasking, high processing speed and a budget of 100000.

The values currently in the string above are examples only.
...
Here is the input string to process:
\"\"\"
{response_assistant}
\"\"\"
"""
    ...
    response = co.chat(
        model="command-r-plus",
        message="Please provide the formatted requirement string.",
        chat_history=chat_history
    )
...
def get_chat_completions_func_calling(input, include_budget):
    ...
    # Prepare chat history (you can keep previous messages if needed)
    chat_history = [
        # Example initial system prompt as first message
        {"user_name": "system", "text": "You are a helpful assistant."}
        # You can add previous conversation messages here if needed
    ]

    # Send user input to Cohere chat endpoint
    response = co.chat(
        model="command-r-plus",
        message=user_input,
        chat_history=chat_history
    ...
    # Cohere response text (expected to be a JSON string with keys)
    text = response.text.strip()

    # Try parsing the JSON string from Cohere output
    try:
        function_parameters = json.loads(text)
    except json.JSONDecodeError:
        raise ValueError("Cohere did not return a valid JSON string with laptop attributes.")

    # Extract budget if needed
    budget = 0
    if include_budget and 'Budget' in function_parameters:
        budget = function_parameters['Budget']

    # Call your existing extraction function with parsed values
    return extract_user_info(
        function_parameters.get('GPU intensity', 'medium'),
        function_parameters.get('Display quality', 'medium'),
        function_parameters.get('Portability', 'medium'),
        function_parameters.get('Multitasking', 'medium'),
        function_parameters.get('Processing speed', 'medium'),
        budget
    )
#
...
def compare_laptops_with_user(user_requirements):
    """
    This function filters and ranks laptops based on user requirements.

    Steps:
    1. Reads a CSV file ('laptopData.csv) containing laptop details.
    2. Applies a function `product_map_layer()` on the 'Description' column to extract relevant features.
    3. Filters laptops within the user's budget.
    4. Compares user requirements with each laptop’s features.
    5. Ranks laptops based on the match percentage.
    6. Returns the top-ranked laptop recommendation.

    Parameters:
    user_requirements (dict): Dictionary containing user preferences for laptop attributes 
                              (GPU intensity, Display quality, Portability, Multitasking, Processing speed, Budget).

    Returns:
    str: The name and details of the best-matching laptop recommendation.
    """
...
def product_map_layer(laptop_description):
    """
    This function analyzes the product description of a laptop and extracts key features 
    (such as GPU intensity, Display quality, Portability, etc.).

    The extracted values will be used to compare with user requirements.

    Parameters:
    description (str): Text description of the laptop's specifications.

    Returns:
    dict: Dictionary containing extracted laptop attributes in the format:
          {'GPU intensity': 'high', 'Display quality': 'medium', 'Portability': 'low', ...}
    """
    delimiter = "#####"
